	...
	switch( Keys ) {
		...
		case	'9':
			if( m_DispLines ) {
				...
				BYTE	data = GlobalMMU.GetCPU_MEM_BANK(addr);
				if( !(m_CursorX&1) ) {
					data &= 0x0F;
					data |= (Keys-'0')<<4;
				} else {
				...
				GlobalMMU.GetCPU_MEM_BANK(addr) = data;
...
void	CMemoryView::OnDraw( HDC hDC )
{
	...
	for( i = 0; i < m_DispLines; i++ ) {
		...
		for( INT d = 0; d < 16; d++ ) {
			CHAR	szTemp[16];
			INT	addr = address+d;
			::wsprintf( szTemp, "%02X ", GlobalMMU.GetCPU_MEM_BANK(addr) );
			::strcat( szBuf, szTemp );
		}
		::strcat( szBuf, " " );
		for( INT a = 0; a < 16; a++ ) {
			...
			if( m_logFont.lfCharSet == SHIFTJIS_CHARSET ) {
				::wsprintf( szTemp, "%1c", ::_ismbcprint(GlobalMMU.GetCPU_MEM_BANK(addr))?GlobalMMU.GetCPU_MEM_BANK(addr):'.' );
			} else {
				::wsprintf( szTemp, "%1c", ::isprint(GlobalMMU.GetCPU_MEM_BANK(addr))?GlobalMMU.GetCPU_MEM_BANK(addr):'.' );
			}
			::strcat( szBuf, szTemp );
		}
