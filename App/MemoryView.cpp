	...
	switch( Keys ) {
		...
		case	'9':
			if( m_DispLines ) {
				...
				BYTE	data = GlobalMMU.CPU_MEM_BANK[addr>>13][addr&0x1FFF];
				if( !(m_CursorX&1) ) {
					data &= 0x0F;
					data |= (Keys-'0')<<4;
				} else {
				...
				GlobalMMU.CPU_MEM_BANK[addr>>13][addr&0x1FFF] = data;
...
void	CMemoryView::OnDraw( HDC hDC )
{
	...
	for( i = 0; i < m_DispLines; i++ ) {
		...
		for( INT d = 0; d < 16; d++ ) {
			CHAR	szTemp[16];
			INT	addr = address+d;
			::wsprintf( szTemp, "%02X ", GlobalMMU.CPU_MEM_BANK[addr>>13][addr&0x1FFF] );
			::strcat( szBuf, szTemp );
		}
		::strcat( szBuf, " " );
		for( INT a = 0; a < 16; a++ ) {
			...
			if( m_logFont.lfCharSet == SHIFTJIS_CHARSET ) {
				::wsprintf( szTemp, "%1c", ::_ismbcprint(GlobalMMU.CPU_MEM_BANK[addr>>13][addr&0x1FFF])?GlobalMMU.CPU_MEM_BANK[addr>>13][addr&0x1FFF]:'.' );
			} else {
				::wsprintf( szTemp, "%1c", ::isprint(GlobalMMU.CPU_MEM_BANK[addr>>13][addr&0x1FFF])?GlobalMMU.CPU_MEM_BANK[addr>>13][addr&0x1FFF]:'.' );
			}
			::strcat( szBuf, szTemp );
		}
