...
//	DEBUGOUT( "CPaletteEdit::OnInitDialog\n" );
	...
	::memcpy( m_BGPAL, GlobalMMU.BGPAL, 16 );
	::memcpy( m_SPPAL, GlobalMMU.SPPAL, 16 );
...
//	DEBUGOUT( "CPaletteEdit::OnLButtonDown\n" );
	...
	if( xp >= rc.left && xp < rc.right && yp >= rc.top && yp < rc.bottom ) {
		...
		if( y == 0 ) m_PaletteSelect = GlobalMMU.BGPAL[x];
		else	     m_PaletteSelect = GlobalMMU.SPPAL[x];
...
//	DEBUGOUT( "CPaletteEdit::OnRButtonDown\n" );
	...
	if( xp >= rc.left && xp < rc.right && yp >= rc.top && yp < rc.bottom ) {
		...
		int	nSel;
		if( y == 0 ) nSel = GlobalMMU.BGPAL[x];
		else	     nSel = GlobalMMU.SPPAL[x];
...
{
	if( ::memcmp( m_BGPAL, GlobalMMU.BGPAL, 16 ) != 0 || ::memcmp( m_SPPAL, GlobalMMU.SPPAL, 16 ) != 0 ) {
		::memcpy( m_BGPAL, GlobalMMU.BGPAL, 16 );
		::memcpy( m_SPPAL, GlobalMMU.SPPAL, 16 );
