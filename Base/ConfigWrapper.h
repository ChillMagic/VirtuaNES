...
enum Keyboard
{
	...
	NUMPAD3 = 0x51,
	NUMPAD0 = 0x52,
	_DECIMAL = 0x53,    /* . on numeric keypad */
...
public:
	...
	static void DirectDrawSetMessageString(LPCSTR str);
	static void DirectDrawSetZapperMode(BOOL bZapper);
	static void DirectDrawSetZapperDrawMode(BOOL bDraw);
	static BYTE DirectDrawGetZapperHit();
	static void DirectDrawSetZapperPos(LONG x, LONG y);
	static void DirectDrawGetPaletteData(RGBQUAD* rgb);

	static BYTE DirectInputGetm_Sw(Keyboard key);

	static BOOL DirectSoundEsfPlay(INT no);
	static BOOL DirectSoundEsfPlayLoop(INT no);
	static BOOL DirectSoundEsfStop(INT no);
	static void DirectSoundEsfAllStop();
};
