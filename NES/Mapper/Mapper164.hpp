...
void	Mapper164::SetBank_CPU()
{
	...
	switch(mode){
		...
		case 6:				/* NORMAL MODE */
			bank = (reg5000&0x0f);
			bank += (reg5000&0x20)>>1;
			nes->mmu.SetPROM_16K_Bank(4, bank+base);
			nes->mmu.SetPROM_16K_Bank(6, base+0x1f);
		...
		case 5:				/* 32K MODE */
			bank = (reg5000&0x0f);
			nes->mmu.SetPROM_32K_Bank(bank+(base>>1));
...
void	Mapper164::PPU_ExtLatch( WORD ntbladr, BYTE& chr_l, BYTE& chr_h, BYTE& attr )
{
	...
	if(p_mode){
		...
		chr_l = chr_h = nes->mmu.PPU_MEM_BANK[tileadr>>10][ tileadr&0x03FF   ];
	}else{
