...
void	Mapper016::Reset()
{
	...
	if( crc == 0x3f15d20d		// Famicom Jump 2(J)
	 || crc == 0xf76aa523 ) {	// Famicom Jump 2(J)(alt)
		...
		nes->mmu.WRAM[0x0BBC] = 0xFF;	// SRAM対策
	}
	...
	if( eeprom_type == 0 ) {
		...
		x24c01.Reset( nes->mmu.WRAM );
	} else 
	if( eeprom_type == 1 ) {
		nes->SetSAVERAM_SIZE( 256 );
		x24c02.Reset( nes->mmu.WRAM );
	} else 
	if( eeprom_type == 2 ) {
		nes->SetSAVERAM_SIZE( 384 );
		x24c02.Reset( nes->mmu.WRAM );
		x24c01.Reset( nes->mmu.WRAM+256 );
	}
}
...
void	Mapper016::WriteSubB( WORD addr, BYTE data )
{
	switch( addr ) {
		...
		case	0x8003:
			reg[0] = data & 0x01;
			nes->mmu.SetPROM_8K_Bank( 4, reg[0]*0x20+reg[2]*2+0 );
			nes->mmu.SetPROM_8K_Bank( 5, reg[0]*0x20+reg[2]*2+1 );
			break;
		case	0x8004:
		case	0x8005:
		case	0x8006:
		case	0x8007:
			reg[1] = data & 0x01;
			nes->mmu.SetPROM_8K_Bank( 6, reg[1]*0x20+0x1E );
			nes->mmu.SetPROM_8K_Bank( 7, reg[1]*0x20+0x1F );
			break;
		case	0x8008:
			reg[2] = data;
			nes->mmu.SetPROM_8K_Bank( 4, reg[0]*0x20+reg[2]*2+0 );
