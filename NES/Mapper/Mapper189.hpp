...
void	Mapper189::Reset()
{
	...
	if( crc == 0x20ca2ad3 ) {	// Street Fighter IV (GOUDER)
		...
		// $4000-$5FFF
		nes->mmu.SetPROM_Bank( 2, nes->mmu.XRAM, BANKTYPE_ROM );
	}
}
...
void	Mapper189::WriteLow( WORD addr, BYTE data )
{
	...
	// Street Fighter IV (GOUDER)
		...
		if( (addr >= 0x4800) && (addr <= 0x4FFF) ) {
			nes->mmu.SetPROM_32K_Bank( ((data&0x10)>>3)+(data&0x1) );

			if( !nes->rom->Is4SCREEN() ) {
				if( data & 0x20 ) nes->mmu.SetVRAM_Mirror( VRAM_HMIRROR );
...
//			nes->mmu.XRAM[0x1000+(addr & 3)] = 
			// $5800 "JMP $xxxx" write
			nes->mmu.XRAM[0x1800+(addr & 3)] = 
