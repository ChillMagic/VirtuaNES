...
void	Mapper235::Write( WORD addr, BYTE data )
{
	...
	} else if( nes->mmu.PROM_8K_SIZE == 128*2 ) {
		// 150-in-1
		switch( addr & 0x0300 ) {
			case	0x0000:	break;
			case	0x0100:	bus = 1; break;
	...
	} else if( nes->mmu.PROM_8K_SIZE == 192*2 ) {
		// 150-in-1
		switch( addr & 0x0300 ) {
			case	0x0000:	break;
			case	0x0100:	bus = 1; break;
	...
	} else if( nes->mmu.PROM_8K_SIZE == 256*2 ) {
	}

	if( addr & 0x0800 ) {
		if( addr & 0x1000 ) {
			nes->mmu.SetPROM_8K_Bank( 4, prg*4+2 );
			nes->mmu.SetPROM_8K_Bank( 5, prg*4+3 );
		...
		nes->mmu.SetPROM_32K_Bank( prg );
	}

	if( bus ) {
		nes->mmu.SetPROM_Bank( 4, nes->mmu.DRAM, BANKTYPE_ROM );
		nes->mmu.SetPROM_Bank( 5, nes->mmu.DRAM, BANKTYPE_ROM );
		nes->mmu.SetPROM_Bank( 6, nes->mmu.DRAM, BANKTYPE_ROM );
		nes->mmu.SetPROM_Bank( 7, nes->mmu.DRAM, BANKTYPE_ROM );
	}
