//////////////////////////////////////////////////////////////////////////
// Mapper235  150-in-1                                                  //
//////////////////////////////////////////////////////////////////////////
void	Mapper235::Reset()
{
	for( INT i = 0; i < 0x2000; i++ ) {
		MMU.DRAM[i] = 0xFF;
	}

	MMU.SetPROM_32K_Bank( 0 );
}

void	Mapper235::Write( WORD addr, BYTE data )
{
	BYTE	prg = ((addr&0x0300)>>3)|(addr&0x001F);
	BYTE	bus = 0;

	if( MMU.PROM_8K_SIZE == 64*2 ) {
		// 100-in-1
		switch( addr & 0x0300 ) {
			case	0x0000:	break;
			case	0x0100:	bus = 1; break;
			case	0x0200:	bus = 1; break;
			case	0x0300:	bus = 1; break;
		}
	} else if( MMU.PROM_8K_SIZE == 128*2 ) {
		// 150-in-1
		switch( addr & 0x0300 ) {
			case	0x0000:	break;
			case	0x0100:	bus = 1; break;
			case	0x0200:	prg = (prg&0x1F)|0x20; break;
			case	0x0300:	bus = 1; break;
		}
	} else if( MMU.PROM_8K_SIZE == 192*2 ) {
		// 150-in-1
		switch( addr & 0x0300 ) {
			case	0x0000:	break;
			case	0x0100:	bus = 1; break;
			case	0x0200:	prg = (prg&0x1F)|0x20; break;
			case	0x0300:	prg = (prg&0x1F)|0x40; break;
		}
	} else if( MMU.PROM_8K_SIZE == 256*2 ) {
	}

	if( addr & 0x0800 ) {
		if( addr & 0x1000 ) {
			MMU.SetPROM_8K_Bank( 4, prg*4+2 );
			MMU.SetPROM_8K_Bank( 5, prg*4+3 );
			MMU.SetPROM_8K_Bank( 6, prg*4+2 );
			MMU.SetPROM_8K_Bank( 7, prg*4+3 );
		} else {
			MMU.SetPROM_8K_Bank( 4, prg*4+0 );
			MMU.SetPROM_8K_Bank( 5, prg*4+1 );
			MMU.SetPROM_8K_Bank( 6, prg*4+0 );
			MMU.SetPROM_8K_Bank( 7, prg*4+1 );
		}
	} else {
		MMU.SetPROM_32K_Bank( prg );
	}

	if( bus ) {
		MMU.SetPROM_Bank( 4, MMU.DRAM, BANKTYPE_ROM );
		MMU.SetPROM_Bank( 5, MMU.DRAM, BANKTYPE_ROM );
		MMU.SetPROM_Bank( 6, MMU.DRAM, BANKTYPE_ROM );
		MMU.SetPROM_Bank( 7, MMU.DRAM, BANKTYPE_ROM );
	}

	if( addr & 0x0400 ) {
		MMU.SetVRAM_Mirror( VRAM_MIRROR4L );
	} else if( addr & 0x2000 ) {
		MMU.SetVRAM_Mirror( VRAM_HMIRROR );
	} else {
		MMU.SetVRAM_Mirror( VRAM_VMIRROR );
	}
}
