...
void	Mapper236::Write( WORD addr, BYTE data )
{
	...
	switch( mode ) {
		case	0x00:
			...
			nes->mmu.SetPROM_8K_Bank( 6, (bank|0x07)*2+0 );
			nes->mmu.SetPROM_8K_Bank( 7, (bank|0x07)*2+1 );
			break;
		case	0x10:
			bank |= 0x37;
			nes->mmu.SetPROM_8K_Bank( 4, bank*2+0 );
		...
		case	0x20:
			bank |= 0x08;
			nes->mmu.SetPROM_8K_Bank( 4, (bank&0xFE)*2+0 );
			nes->mmu.SetPROM_8K_Bank( 5, (bank&0xFE)*2+1 );
			nes->mmu.SetPROM_8K_Bank( 6, (bank&0xFE)*2+2 );
			nes->mmu.SetPROM_8K_Bank( 7, (bank&0xFE)*2+3 );
			break;
		case	0x30:
			bank |= 0x08;
			nes->mmu.SetPROM_8K_Bank( 4, bank*2+0 );
