...
void	Mapper112::Write( WORD addr, BYTE data )
{
	switch( addr ) {
		...
		case	0xA000:
			...
			switch( reg[0] & 0x07 ) {
				case	0x00:
					prg0 = (data&(nes->mmu.PROM_8K_SIZE-1));
					SetBank_CPU();
					break;
				case	0x01:
					prg1 = (data&(nes->mmu.PROM_8K_SIZE-1));
					SetBank_CPU();
					break;
				case	0x02:
					chr01 = data & 0xFE;
...
{
	if( reg[2] & 0x02 ) {
		nes->mmu.SetVROM_8K_Bank( chr01, chr01+1, chr23, chr23+1, chr4, chr5, chr6, chr7 );
	} else {
		nes->mmu.SetVROM_8K_Bank(((reg[3]<<6)&0x100)+chr01, 
