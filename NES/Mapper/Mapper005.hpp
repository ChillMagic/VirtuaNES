...
INT	i;
	...
	for( i = 0; i < 8; i++ ) {
		BG_MEM_BANK[i] = nes->mmu.VROM+0x0400*i;
		BG_MEM_PAGE[i] = i;
	}
...
//			irq_status = 0;
	...
	if( addr >= 0x5C00 && addr <= 0x5FFF ) {
		if( graphic_mode >= 2 ) { // ExRAM mode
			data = nes->mmu.VRAM[0x0800+(addr&0x3FF)];
		}
	...
	switch( addr ) {
		...
		case	0x5105:
			...
			for( i = 0; i < 4; i++ ) {
				...
				nes->mmu.SetVRAM_1K_Bank( 8+i, nametable_type[i] );
				data >>= 2;
			}
			break;
		...
		default:
			if( addr >= 0x5000 && addr <= 0x5015 ) {
				...
				if( graphic_mode == 2 ) {		// ExRAM
					nes->mmu.VRAM[0x0800+(addr&0x3FF)] = data;
				} else if( graphic_mode != 3 ) {	// Split,ExGraphic
					if( irq_status&0x40 ) {
						nes->mmu.VRAM[0x0800+(addr&0x3FF)] = data;
					} else {
						nes->mmu.VRAM[0x0800+(addr&0x3FF)] = 0;
					}
				}
			} else if( addr >= 0x6000 && addr <= 0x7FFF ) {
				if( (sram_we_a == 0x02) && (sram_we_b == 0x01) ) {
					if( nes->mmu.CPU_MEM_TYPE[3] == BANKTYPE_RAM ) {
						nes->mmu.CPU_MEM_BANK[3][addr&0x1FFF] = data;
					}
				}
			}
			break;
...
void	Mapper005::SetBank_CPU( WORD addr, BYTE data )
{
	...
	// PROM Bank
		switch( addr & 7 ) {
			case	4:
				if( prg_size == 3 ) {
					nes->mmu.SetPROM_8K_Bank( 4, data&0x7F );
				}
				break;
			case	5:
				if( prg_size == 1 || prg_size == 2 ) {
					nes->mmu.SetPROM_16K_Bank( 4, (data&0x7F)>>1 );
				} else if( prg_size == 3 ) {
					nes->mmu.SetPROM_8K_Bank( 5, (data&0x7F) );
				}
				break;
			case	6:
				if( prg_size == 2 || prg_size == 3 ) {
					nes->mmu.SetPROM_8K_Bank( 6, (data&0x7F) );
				}
				break;
			case	7:
				if( prg_size == 0 ) {
					nes->mmu.SetPROM_32K_Bank( (data&0x7F)>>2 );
				} else if( prg_size == 1 ) {
					nes->mmu.SetPROM_16K_Bank( 6, (data&0x7F)>>1 );
				} else if( prg_size == 2 || prg_size == 3 ) {
					nes->mmu.SetPROM_8K_Bank( 7, (data&0x7F) );
				}
				break;
		}
	} else {
...
void	Mapper005::SetBank_SRAM( BYTE page, BYTE data )
{
	...
	if( data != 8 ) {
		nes->mmu.SetPROM_Bank( page, &nes->mmu.WRAM[0x2000*data], BANKTYPE_RAM );
		nes->mmu.CPU_MEM_PAGE[page] = data;
	} else {
		nes->mmu.CPU_MEM_TYPE[page] = BANKTYPE_ROM;
	}
}
...
INT	i;
	...
	// PPU SP Bank
		switch( chr_size ) {
			case	0:
				nes->mmu.SetVROM_8K_Bank( chr_page[0][7] );
				break;
			case	1:
				nes->mmu.SetVROM_4K_Bank( 0, chr_page[0][3] );
				nes->mmu.SetVROM_4K_Bank( 4, chr_page[0][7] );
				break;
			case	2:
				nes->mmu.SetVROM_2K_Bank( 0, chr_page[0][1] );
				nes->mmu.SetVROM_2K_Bank( 2, chr_page[0][3] );
				nes->mmu.SetVROM_2K_Bank( 4, chr_page[0][5] );
				nes->mmu.SetVROM_2K_Bank( 6, chr_page[0][7] );
				break;
			case	3:
				nes->mmu.SetVROM_8K_Bank( chr_page[0][0],
						 chr_page[0][1],
						 chr_page[0][2],
						 chr_page[0][3],
						 chr_page[0][4],
		...
		switch( chr_size ) {
			case	0:
				for( i = 0; i < 8; i++ ) {
					BG_MEM_BANK[i] = nes->mmu.VROM+0x2000*(chr_page[1][7]%nes->mmu.VROM_8K_SIZE)+0x0400*i;
					BG_MEM_PAGE[i] = (chr_page[1][7]%nes->mmu.VROM_8K_SIZE)*8+i;
				}
				break;
			case	1:
				for( i = 0; i < 4; i++ ) {
					BG_MEM_BANK[i+0] = nes->mmu.VROM+0x1000*(chr_page[1][3]%nes->mmu.VROM_4K_SIZE)+0x0400*i;
					BG_MEM_BANK[i+4] = nes->mmu.VROM+0x1000*(chr_page[1][7]%nes->mmu.VROM_4K_SIZE)+0x0400*i;
					BG_MEM_PAGE[i+0] = (chr_page[1][3]%nes->mmu.VROM_4K_SIZE)*4+i;
					BG_MEM_PAGE[i+4] = (chr_page[1][7]%nes->mmu.VROM_4K_SIZE)*4+i;
				}
				break;
			case	2:
				for( i = 0; i < 2; i++ ) {
					BG_MEM_BANK[i+0] = nes->mmu.VROM+0x0800*(chr_page[1][1]%nes->mmu.VROM_2K_SIZE)+0x0400*i;
					BG_MEM_BANK[i+2] = nes->mmu.VROM+0x0800*(chr_page[1][3]%nes->mmu.VROM_2K_SIZE)+0x0400*i;
					BG_MEM_BANK[i+4] = nes->mmu.VROM+0x0800*(chr_page[1][5]%nes->mmu.VROM_2K_SIZE)+0x0400*i;
					BG_MEM_BANK[i+6] = nes->mmu.VROM+0x0800*(chr_page[1][7]%nes->mmu.VROM_2K_SIZE)+0x0400*i;
					BG_MEM_PAGE[i+0] = (chr_page[1][1]%nes->mmu.VROM_2K_SIZE)*2+i;
					BG_MEM_PAGE[i+2] = (chr_page[1][3]%nes->mmu.VROM_2K_SIZE)*2+i;
					BG_MEM_PAGE[i+4] = (chr_page[1][5]%nes->mmu.VROM_2K_SIZE)*2+i;
					BG_MEM_PAGE[i+6] = (chr_page[1][7]%nes->mmu.VROM_2K_SIZE)*2+i;
				}
				break;
			case	3:
				for( i = 0; i < 8; i++ ) {
					BG_MEM_BANK[i] = nes->mmu.VROM+0x0400*(chr_page[1][i]%nes->mmu.VROM_1K_SIZE);
					BG_MEM_PAGE[i] = (chr_page[1][i]%nes->mmu.VROM_1K_SIZE)+i;
				}
				break;
		}
	}
	...
	if( !bSplit ) {
		...
		// Fill mode
			...
			// ExGraphic mode
				...
				// Get TileBank
				tilebank = 0x1000*((nes->mmu.VRAM[0x0800+(ntbladr&0x03FF)]&0x3F)%nes->mmu.VROM_4K_SIZE);
				// Attribute
				attr = (fill_pal<<2)&0x0C;
				// Get Pattern
				chr_l = nes->mmu.VROM[tilebank+tileadr  ];
				chr_h = nes->mmu.VROM[tilebank+tileadr+8];
			} else {
			// Normal
				tileofs = (nes->mmu.PPUREG[0]&PPU_BGTBL_BIT)?0x1000:0x0000;
		...
		// ExGraphic mode
			ntbladr = 0x2000+(addr&0x0FFF);
			// Get Nametable
			tileadr = (WORD)nes->mmu.PPU_MEM_BANK[ntbladr>>10][ntbladr&0x03FF]*0x10+tile_yofs;
			// Get TileBank
			tilebank = 0x1000*((nes->mmu.VRAM[0x0800+(ntbladr&0x03FF)]&0x3F)%nes->mmu.VROM_4K_SIZE);
			// Get Attribute
			attr = (nes->mmu.VRAM[0x0800+(ntbladr&0x03FF)]&0xC0)>>4;
			// Get Pattern
			chr_l = nes->mmu.VROM[tilebank+tileadr  ];
			chr_h = nes->mmu.VROM[tilebank+tileadr+8];
		} else {
		// Normal or ExVRAM
			...
			// Get Nametable
			tileadr = tileofs+nes->mmu.PPU_MEM_BANK[ntbladr>>10][ntbladr&0x03FF]*0x10+tile_yofs;
			// Get Attribute
			attr = nes->mmu.PPU_MEM_BANK[attradr>>10][attradr&0x03FF];
		...
		// Get Split TileBank
		tilebank = 0x1000*((INT)split_page%nes->mmu.VROM_4K_SIZE);
		tileadr  = (WORD)nes->mmu.VRAM[0x0800+ntbladr]*0x10+split_yofs;
		// Get Attribute
		attradr = 0x03C0+((ntbladr&0x0380)>>4)+((ntbladr&0x001C)>>2);
		attr = nes->mmu.VRAM[0x0800+attradr];
		if( ntbladr & 0x0002 ) attr >>= 2;
		if( ntbladr & 0x0040 ) attr >>= 4;
		attr = (attr&0x03)<<2;
		// Get Pattern
...
//	// BGバンクの再設定処理
//	for( i = 0; i < 8; i++ ) {
//		BG_MEM_PAGE[i] = p[40+i]%nes->mmu.VROM_1K_SIZE;
//	}
//	for( i = 0; i < 8; i++ ) {
//		BG_MEM_BANK[i] = nes->mmu.VROM+0x0400*BG_MEM_PAGE[i];
//	}
